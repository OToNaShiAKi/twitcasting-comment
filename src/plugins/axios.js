...
Baidu.interceptors.response.use(({ data, ...others }) => {
...
/* (async () => {
  try {
    ...
    const result = await axios.get("https://fanyi.baidu.com", {
      headers: Baidu.defaults.headers,
      withCredentials: true,
    });
    Baidu.defaults.headers.Cookie +=
      ";" +
      result.headers["set-cookie"].map((item) => item.split(";")[0]).join(";");
